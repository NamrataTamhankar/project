...
$PARAMETER_FILE = $Properties.'PARAMETER_FILE_IMNa_BESI_Export_Submission'
$AUTOMATION_NAME = 'IMNa_BESI_Export_Submission'
...
# Define a list of workspaces to exclude
        ...
        $excludeWorkspaces = @('IMNa_BESI_UpdateDatamarts.fmw', 'IMNa_BESI_Update_BasisGrid.fmw')
	try{
		Get-ChildItem "$REPOSITORY_LOCATION"  | 
		Foreach-Object {
			$folder = $_.Name
			if ( 'IMNa_BESI' -eq $folder ){
