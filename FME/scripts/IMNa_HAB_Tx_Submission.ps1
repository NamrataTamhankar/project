...
$PARAMETER_FILE = $Properties.'PARAMETER_FILE_IMNa_HAB_Tx_Submission'
$AUTOMATION_NAME = 'IMNa_HAB_Tx_Submission'
...
# Define a list of workspaces to exclude
	$excludeWorkspaces = @('IMNa_HAB_T0_GenerateFGDBFile.fmw')
	try{
		Get-ChildItem "$REPOSITORY_LOCATION"  | 
		Foreach-Object {
			$folder = $_.Name
			if ( 'IMNa_HAB' -eq $folder ){
